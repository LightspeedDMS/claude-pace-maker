...
### Changed
- **Managed Python venv with pinned `requirements.txt`** — Bootstrap no longer runs `pip install --user` against the system interpreter (which fails on macOS Homebrew and other PEP 668 environments). Pinned dependencies live in `requirements.txt` at the repo root (single source of truth, consumable by `pip install -r`, dependency scanners, dependabot, and GitHub's dependency graph). They are installed into `~/.claude-pace-maker/venv` via `pip install -r requirements.txt` — idempotent (no-op when satisfied, installs missing, replaces drifted). Hooks and the `pace-maker` CLI use that venv via `resolve_runtime_python()`. `_deps_imports_ok` asserts every spec from `requirements.txt` is installed at its exact pinned version on every runtime resolution, so a drifting venv (manual upgrade, partial install) is detected and repaired on the next `bootstrap_full`.
  - `requirements.txt` — `requests==2.34.2`, `pyyaml==6.0.3`, `claude-agent-sdk==0.2.87`. Shipped in the plugin cache (full repo clone) and the pipx wheel (via `MANIFEST.in`). Bumping a pin or editing the file in any way rolls its sha256, which is the suffix of `.venv_stamp` — `bootstrap_needs_full` detects the mismatch on the next hook and re-installs automatically.
  - `scripts/bootstrap-plugin.sh` — `_ensure_pinned_deps_loaded()` lazily parses `requirements.txt` into a `PINNED_PACKAGES` bash array and computes `DEPS_SIGNATURE` as `sha256(requirements.txt)` (using `sha256sum`/`shasum`/`openssl` — whichever the host has). `_ensure_venv_and_deps()` has a fast path (no lock when the venv is already healthy) and a slow path (`_create_or_repair_venv_locked()` under the install lock) that owns every venv mutation (`rm -rf`, `python -m venv`, `pip install -r`). Portable install lock: `flock` on Linux, `symlink(2)`-based lock at `.venv.lock.link` elsewhere — `ln -s "$$" $VENV_LOCK_LINK` binds the holder's pid into the link target in one atomic syscall, so the lock identity and the holder identity are bound into a single observable name; `readlink` + `kill -0` detects stale holders.
  ...
  - `scripts/hook.sh` — prefers the managed venv over system Python via `resolve_hook_python()`. When the venv is unavailable (missing or deps not importable at the pinned versions), the fallback to system Python is logged loudly to `hook_debug.log` (named interpreter, reason, `pace-maker doctor` pointer); throttled to once per hour via `.python_fallback_warn` and cleared on successful `bootstrap_full` so a future regression re-warns.
  - `MANIFEST.in` — ships `requirements.txt` in the pipx-built sdist/wheel.
- **Cheap `bootstrap_needs_full` for per-hook calls** — Replaces the per-hook deep import check (3-module python fork) with a stat + single sha256 of `requirements.txt` (no python fork): `BOOTSTRAP_OK_MARKER` exists, `VENV_PYTHON` is executable, and `.venv_stamp` suffix equals the file's sha256. The deep import + exact-version-pin check still runs on `session_start` via `bootstrap_full` → `_ensure_venv_and_deps`.

### Tests (`tests/test_bootstrap_plugin.py`, `tests/test_plugin_lazy_init.py`)
- `TestBootstrapVenv` — venv created with the pinned interpreter; second `--full` is idempotent; `.venv_stamp` records base python and the sha256 of `requirements.txt`; manually downgrading a dep (`pip install requests==2.32.0`) makes `resolve_runtime_python` reject the venv and the next `bootstrap_full` repairs it; editing `requirements.txt` (simulated by writing a wrong-sha stamp) makes `bootstrap_needs_full` report "needs full"
...
- `TestBootstrapNeedsFullIsCheap` — `bootstrap_needs_full` never forks the venv python (sentinel-based assertion); reports "needs full" when stamp signature doesn't match
...
- Bumping a dependency version is a one-line edit in `requirements.txt`; existing installs detect the new sha256 on the next hook invocation and re-install automatically.
