...
class TestThrottlingIntegration(unittest.TestCase):
    ...
    def test_throttling_prompt_injection_when_credits_low(self):
        ...
        original_run_pacing_check = pacing_engine.run_pacing_check
        ...
        with patch.object(hook, 'DEFAULT_CONFIG_PATH', self.config_path), \
             ...
             patch.object(pacing_engine, 'run_pacing_check', side_effect=mock_pacing_check), \
             patch('sys.stdout', captured_stdout), \
             patch.object(sys, 'argv', ['hook.py', 'post_tool_use']):
        ...
        self.assertIn('[PACING]', output,
                     "Throttling prompt must be injected to stdout")
        self.assertIn('wait', output.lower(),
                     "Prompt must mention waiting")
        self.assertIn('120 seconds', output,
                     "Prompt must specify delay duration")
    ...
    def test_no_throttling_when_credits_healthy(self):
        ...
        self.assertEqual(output, "",
                        "No throttling output when credits are healthy")
    ...
    def test_direct_delay_for_small_deviations(self):
        ...
        self.assertGreaterEqual(elapsed, 2.0,
                               "Direct delay must actually sleep")

        # Verify no output (silent delay)
        output = captured_stdout.getvalue()
        self.assertEqual(output, "",
                        "Direct delay produces no output")
    ...
    def test_disabled_pacemaker_produces_no_throttling(self):
        ...
        output = captured_stdout.getvalue()
        self.assertEqual(output, "",
                        "Disabled pace maker produces no output")
