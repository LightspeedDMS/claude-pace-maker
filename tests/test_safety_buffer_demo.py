...
def demo_safety_buffer():
    ...
    for scenario in scenarios:
        ...
        print(
            f"  Safe Allowance:    {proj['safe_allowance']:.1f}% (95% of {proj['allowance']:.1f}%)"
        )
...
def demo_custom_buffers():
    ...
    print("Raw allowance: ~50% (Wednesday noon)")
    print()
    ...
    for buffer_pct, description in buffers:
        ...
        print(
            f"  Throttle? {'YES' if delay > 0 else 'NO'} (delay: {delay}s, strategy: {strategy})"
        )
        print()
