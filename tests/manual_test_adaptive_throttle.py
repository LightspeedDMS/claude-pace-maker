...
def print_scenario(name, params, description=""):
    ...
    print("\nAlgorithm Decision:")
    print(
        f"  Delay: {result['delay_seconds']} seconds ({result['delay_seconds']/60:.1f} minutes)"
    )
    print(f"  Strategy: {result['strategy']}")

    print("\nProjection:")
    print(
        f"  Without throttling -> {result['projection']['util_if_no_throttle']:.1f}% at window end"
    )
    print(
        f"  With throttling    -> {result['projection']['util_if_throttled']:.1f}% at window end"
    )
    print(
        f"  Estimated tools remaining: {result['projection']['tools_remaining_estimate']}"
    )
    ...
    if result["projection"]["util_if_no_throttle"] > 100:
        print(
            f"WARNING: Without throttling, would EXCEED budget by "
            f"{result['projection']['util_if_no_throttle'] - 100:.1f}%!"
        )
    else:
        print(
            f"INFO: Without throttling, would end at {result['projection']['util_if_no_throttle']:.1f}% (within budget)"
        )
