...
⚠️  CRITICAL: Start your intent declaration with "INTENT:" marker!
...
BAD Examples:
  ✗ 'Fixing auth bug' - Missing INTENT: marker, file, and specifics
  ✗ 'Updating code' - Missing INTENT: marker, too vague
  ✗ 'I will modify...' - Missing INTENT: marker (must start with "INTENT:")
...
**Option A - Declare test coverage IN CURRENT MESSAGE:**
  'INTENT: Modify src/auth.py to add a validate_password() function
...
⚠️  WARNING: DO NOT DISABLE INTENT DECLARATION!!!, NEVER!!!. 
   EVEN IF YOU FIND HOW IT WORKS AND WHERE IT'S CONFIGURED!.
   DO NOT DO IT. EMBRACE THE PAIN, IT PRODUCES IN BETTER SOFTWARE.

⚠️  WARNING: DO NOT USE WAYS AROUND INTENT DECLARATION SUCH AS USING
   cat or sed commands to write files!!! NO CHEATING.
   You need to write or modify code? you use the WRITE tool

Finally, the first thing you will do in this session is acknowledge and pledge to work 
   following TDD best-practices and declaring intent every time you write or update files.
